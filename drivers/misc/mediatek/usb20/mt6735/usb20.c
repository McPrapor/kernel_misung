...
extern struct pinctrl_state *pinctrl_drvvbus_high;
extern struct pinctrl_state *pinctrl_drvvbus_low;
static ssize_t show_mobilesourceControl_value(struct device_driver *ddp, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", mobile_source_flag);
}

static ssize_t store_mobilesourceControl_value(struct device_driver *ddp, const char *buf, size_t count)
{
    ...
    if(!strncmp(buf, "on", 2))
	{
	   ...
	   if (!IS_ERR(pinctrl_drvvbus_high))
				   ...
				   printk("otg: is enable pinctrl_drvvbus_high pinctrl is invalid !!\n");
	}	
	else
	{
		...
		if (!IS_ERR(pinctrl_drvvbus_low))
					...
					printk("otg: is enable pinctrl_drvvbus_low pinctrl is invalid !!\n");
	}
...
#endif
#if defined(CONFIG_USB_MTK_OTG)
		ret =driver_create_file(&mt_usb_driver.driver, &driver_attr_mobilesourceControl);
#endif
