...
#if (__HCT_USB_MTK_OTG_SUPPORT__==HCT_YES)
#else
    #error "kernel defined CONFIG_USB_MTK_OTG, and hct_custom_config.h shold defined __HCT_USB_MTK_OTG_SUPPORT__=HCT_YES"
#endif
...
void mt_usb_set_vbus(struct musb *musb, int is_on)
{
	...
	if (is_on) {
		...
		if (!IS_ERR(pinctrl_drvvbus_high))
			...
			printk("pinctrl_drvvbus_high pinctrl is invalid !!\n");
		...
		if (!IS_ERR(pinctrl_drvvbus_low))
			...
			printk("pinctrl_drvvbus_low pinctrl is invalid !!\n");
	#elif defined(CONFIG_MTK_NCP1851_SUPPORT)
...
void mt_usb_set_vbus_gpio(int state)
{
	if(state == 1)
     	{
#ifdef CONFIG_OF
