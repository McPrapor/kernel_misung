...
#else
#define V1PERCENT_HOLD_TIME 300
#endif
#endif
...
extern void mt_usb_set_vbus_gpio(int state);
					...
					const char *buf, size_t size)
{
	...
	if (rv != 0)
		return -EINVAL;
	battery_log(BAT_LOG_CRTI, "call state = %d\n", g_call_state);
#if defined(__HCT_CONFIG_TPD_PROXIMITY_SPECIAL__)
#if __HCT_CONFIG_TPD_PROXIMITY_SPECIAL__
	tpd_special_enable_ps(g_call_state);
#endif
#endif
	return size;
}
...
#if 1//defined(CONFIG_MTK_DUAL_INPUT_CHARGER_SUPPORT)
			battery_log(BAT_LOG_CRTI,
...
int charger_hv_detect_sw_thread_handler(void *unused)
{
	...
	unsigned int hv_voltage = 10500*1000;
#else
...
static int __batt_init_cust_data_from_dt(void)
{
	...
	np = of_find_compatible_node(NULL, NULL, "mediatek,battery_tmp");
	if (!np) {
...
static int battery_probe(struct platform_device *dev)
{
 ...
 #if 1  // def __HCT_BATTERY_CHARGER_CONTROL__
 //#if __HCT_BATTERY_CHARGER_CONTROL__
	ret_device_file = device_create_file(&(dev->dev), &dev_attr_ChargerStatus);
 //#endif
 #endif
	}
