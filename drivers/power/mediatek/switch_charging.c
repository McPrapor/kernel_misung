...
PMU_STATUS do_charger_state_machine(void)
{
	...
	if (BMT_status.temperature > TEMP_POS_60_THRESHOLD) {
		battery_log(BAT_LOG_CRTI,
			    "[BATTERY] Battery Over high Temperature(%d) !!\n\r",
			    TEMP_POS_60_THRESHOLD);
		g_temp_status = HCT_TEMP_ABOVE_POS_55;
		return PMU_STATUS_FAIL;
	}
	else if(BMT_status.temperature < TEMP_NEG_10_THRESHOLD){
		battery_log(BAT_LOG_CRTI,
			    "[BATTERY] Battery Over high Temperature(%d) !!\n\r",
			    TEMP_POS_60_THRESHOLD);
		g_temp_status = HCT_TEMP_BELOW_NEG_10;
		return PMU_STATUS_FAIL;
	}
	else if (BMT_status.temperature >= (TEMP_POS_60_THRESHOLD - TEMP_DALTA_THRES_DEGREE)) {
		      if(g_temp_status == HCT_TEMP_ABOVE_POS_55){
			battery_log(BAT_LOG_CRTI,
				    "[BATTERY] Battery Temperature from 55 , must blow 52 can rechage,cur=%d,not allow charging yet!!\n\r",
			...
			battery_log(BAT_LOG_CRTI,
				    "[BATTERY] Battery Temperature between %d and %d !!\n\r",
				    TEMP_POS_50_THRESHOLD, TEMP_POS_60_THRESHOLD);
			g_temp_status = HCT_TEMP_POS_50_TO_POS_55;
//			charging_full_current = AC_TEM_PROTECT_CHARGER_CURRENT;
		}
	} else if (BMT_status.temperature >= TEMP_POS_50_THRESHOLD) {
	
		if (g_temp_status == HCT_TEMP_POS_0_TO_POS_50) {
			g_temp_status = HCT_TEMP_POS_50_TO_POS_55;
//			charging_full_current = AC_TEM_PROTECT_CHARGER_CURRENT;
			battery_log(BAT_LOG_CRTI,
				    "[BATTERY] Battery Temperature rasing above 50, need down!!\n\r");
		}
	} else if(BMT_status.temperature >= TEMP_POS_0_THRESHOLD){
	      
		if ((g_temp_status == HCT_TEMP_POS_50_TO_POS_55)&&(BMT_status.temperature <= (TEMP_POS_50_THRESHOLD - TEMP_DALTA_THRES_DEGREE))) {
			g_temp_status = HCT_TEMP_POS_0_TO_POS_50;
//			charging_full_current = batt_cust_data.charging_full_current;;
			battery_log(BAT_LOG_CRTI,
					"[BATTERY] Battery Temperature failng below 47 , return to nomal!!\n\r");
		}
		else if((g_temp_status == HCT_TEMP_NEG_10_TO_POS_0 )&&(BMT_status.temperature <= (TEMP_POS_0_THRESHOLD + TEMP_DALTA_THRES_DEGREE)))
		{
			g_temp_status = HCT_TEMP_POS_0_TO_POS_50;
//			charging_full_current = batt_cust_data.charging_full_current;;
			battery_log(BAT_LOG_CRTI,
					"[BATTERY] Battery Temperature rasing above 0 , return to nomal!!\n\r");
		
		}
	} else if(BMT_status.temperature >= TEMP_NEG_10_THRESHOLD){
		  
		if ((g_temp_status == HCT_TEMP_BELOW_NEG_10)&&(BMT_status.temperature >= (TEMP_NEG_10_THRESHOLD + TEMP_DALTA_THRES_DEGREE))) {
			g_temp_status = HCT_TEMP_NEG_10_TO_POS_0;
//			charging_full_current = AC_TEM_PROTECT_CHARGER_CURRENT;;
			battery_log(BAT_LOG_CRTI,
					"[BATTERY] Battery Temperature rasing above -7 , need down!!!!\n\r");
		}
		else if(g_temp_status == HCT_TEMP_NEG_10_TO_POS_0)
		{
			g_temp_status = HCT_TEMP_POS_0_TO_POS_50;
//			charging_full_current = AC_TEM_PROTECT_CHARGER_CURRENT;;
			battery_log(BAT_LOG_CRTI,
	...
	} else {  // means temprature between -10 to 0
		battery_log(BAT_LOG_CRTI,
			    "[BATTERY] Battery Temperature statu unkhown(%d) , tem_st=%d !!\n\r",
			    BMT_status.temperature,g_temp_status );
		return PMU_STATUS_FAIL;
	}
...
#if __HCT_BATTERY_PROTECT_SUPPORT__
      if(1)//(ghct_temp_status ==HCT_TEMP_STATE_UNKNOWN)  // this is first init
      	{
      	      ...
      	      if(BMT_status.temperature >= TEMP_POS_50_THRESHOLD)
      	      {
...
////--> this is hct defined by hct custom	
#endif	
////--> next  is hct defined by hct custom		
...
static void pchr_turn_on_charging(void)
{
	...
	if (BMT_status.bat_charging_state == CHR_ERROR) {
		...
		if (get_usb_current_unlimited()) {
			...
			battery_log(BAT_LOG_CRTI,
						"[BATTERY] select_charging_curret_bcct not used, but use hct mothod !\n");
