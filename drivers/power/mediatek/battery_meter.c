...
#if 1//defined(CONFIG_HCT_BATTERY_IMPROVE)
#define Jay_M
#endif
/* ============================================================ // */
/* define */
...
#ifdef Jay_M
signed int avg_gFG_capacity_by_c;
signed int avg_gFG_capacity_by_v;
signed int gfg_c_init_flag=0;
signed int gfg_v_init_flag=0;
...
int __batt_meter_init_cust_data_from_dt(void)
{
	...
	np = of_find_compatible_node(NULL, NULL, "mediatek,bat_meter_tmp");
	if (!np) {
...
#if 0//defined(INIT_SOC_BY_SW_SOC)
if (((g_rtc_fg_soc != 0)
		     &&
...
static signed int avg_gFG_capacity_by_c_b[FG_CAPACITY_AVERAGE_SIZE];
static signed int avg_gFG_capacity_by_v_b[FG_CAPACITY_AVERAGE_SIZE];
...
static void mt_fg_capacity_data_init(signed int *bufferdata, signed int data)
{
    int temp_i=0;
    for(;temp_i<FG_CAPACITY_AVERAGE_SIZE;temp_i++)
    {
...
static signed int mt_fg_capacity_insert_c_getavarage(signed int *bufferdata, signed int data, int init_flag)
{
	signed int avgdata;
       static signed int sum_data;
...
static signed int mt_fg_capacity_insert_v_getavarage(signed int *bufferdata, signed int data, int init_flag)
{
	signed int avgdata;
       static signed int sum_data;
       static  unsigned char adc_arry_index;
...
signed int orginal_gFG_voltage = -1;
...
#if defined(__HCT_CLOSE_FG_VOLT_AVG_RUN__)
#else
...
#ifdef Jay_M   //close by hsl  cr id :ALPS02611759  every 10s up a large current
	last_time = 0;
#else
...
#ifdef __HCT_DALTA_COLUME_VALUE__
#define DALTA_COLUME_VALUE   __HCT_DALTA_COLUME_VALUE__
#else
#define DALTA_COLUME_VALUE   110
#endif
#ifdef __HCT_DALTA_COLUME_HOLE_VALUE__
#define DALTA_COLUME_HOLE_VALUE   __HCT_DALTA_COLUME_HOLE_VALUE__
#endif
...
#if 0
static signed int previous_capacity_by_c;
#endif
static signed int previous_decharge_colume;

signed int check_volum_count_decrese(signed int current_colume)
{
     signed int need_decrease=0;
     signed int decrese_colum_percent=0;

     if(previous_decharge_colume >=current_colume)  /* from log : curr_col=-45, pre=-40, */
     {
         decrese_colum_percent =(previous_decharge_colume-current_colume)*100*100/gFG_BATT_CAPACITY_aging ;
         if(decrese_colum_percent > DALTA_COLUME_VALUE) /* this means colume counter measure soc decrase 1.2%, we need decrease soc  */
         {
             need_decrease = 1; 
//             previous_decharge_colume = current_colume;
     ...
     printk("FGADC: check_volum_count_decrese---: curr_col=%d, pre=%d, percent =%d, dec=%d \n", current_colume,previous_decharge_colume,decrese_colum_percent, need_decrease);
     
     return need_decrease;
}
#ifdef DALTA_COLUME_HOLE_VALUE	
signed int check_volum_count_hold(signed int current_colume)
{
     signed int need_hold=1;
     signed int hold_colum_percent=0;

     if(previous_decharge_colume >=current_colume)  /* from log : curr_col=-45, pre=-40, */
     {
     
         hold_colum_percent =(previous_decharge_colume-current_colume)*100*100/gFG_BATT_CAPACITY_aging ;
         if(hold_colum_percent > DALTA_COLUME_HOLE_VALUE) /* this means colume counter measure soc decrase 1.2%, we need decrease soc  */
         {
             need_hold = 0; 
 //            previous_decharge_colume = current_colume;
         }
     ...
     printk("FGADC: check_volum_count_hold---: curr_col=%d, pre=%d, percent =%d, hold=%d \n", current_colume,previous_decharge_colume,hold_colum_percent, need_hold);
