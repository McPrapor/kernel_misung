...
{
	I("%s:Enter \n", __func__);
	printk("[touchdbg] going to check MTK flag\n");
#ifdef MTK
	printk("[touchdbg] MTK flag is defined\n");

#else
	printk("[touchdbg] MTK flag is NOT defined\n");

#endif
#ifdef MTK
	//i2c_register_board_info(TPD_I2C_NUMBER, &himax852xes_i2c, 1);
	I("%s: ifdef MTK = true tpd_get_dts_info going \n", __func__);
	tpd_get_dts_info();
	I("%s: after tpd_get_dts_info \n", __func__);
	if (tpd_driver_add(&tpd_device_driver) < 0) {
		I("%s: tpd_get_dts_info  < 0\n", __func__);
		TPD_DMESG("add hX852Xes driver failed\n");
	} else {
		I("%s: after tpd_get_dts_info > 0 \n", __func__);
