...
int himax_ts_register_interrupt(struct i2c_client *client)
{
	...
	if(HX_INT_IS_EDGE)
        ...
        if(HX_INT_IS_EDGE) {
        	I("%s edge trigger falling\n ",__func__);
                ret = request_irq(client->irq, tpd_eint_interrupt_handler, CUST_EINTF_TRIGGER_FALLING, "TOUCH_PANEL-eint", NULL);
                if(ret > 0){
	                ret = -1;
                ...
                I("%s level trigger low\n ",__func__);
                ret = request_irq(client->irq, tpd_eint_interrupt_handler, CUST_EINTF_TRIGGER_LOW, "TOUCH_PANEL-eint", NULL);
                if(ret > 0){
        		ret = -1;
                        E("tpd request_irq IRQ LINE NOT AVAILABLE\n");
                }
