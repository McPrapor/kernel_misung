...
{
	if (enable == 1 && irq_enable_count == 0) {
		mt_eint_ack(CUST_EINT_TOUCH_PANEL_NUM);
...
void himax_switch_gpio_set(int pinnum, uint8_t value)
{
	mt_set_gpio_mode(GPIO_TP_I2C_SEL, GPIO_TP_I2C_SEL_M_GPIO);
	mt_set_gpio_dir(GPIO_TP_I2C_SEL, GPIO_DIR_OUT);
	if(value)
		mt_set_gpio_out(GPIO_TP_I2C_SEL, GPIO_OUT_ONE);
	else
		mt_set_gpio_out(GPIO_TP_I2C_SEL, GPIO_OUT_ZERO);
