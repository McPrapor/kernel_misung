...
#define CLAM_CLOSE_KEY KEY_BASSBOOST  //only for test
#define CLAM_OPEN_KEY KEY_FASTFORWARD //only for test
...
//extern void long_press_reboot_function_setting(void);
int start_flag=0;
int poweroff_flag=0;

static ssize_t kpd_show_kernelrestart(struct device_driver *ddri, char *buf)
{
	//ssize_t res;

	//res = snprintf(buf, PAGE_SIZE, "%d\n", start_flag);
  sprintf(buf, "%d", start_flag);
	return 1;
}
static ssize_t kpd_store_kernelrestart(struct device_driver *ddri, const char *buf, size_t count)
{
...
//	printk("hsl kpd_store_kernelrestart %d\n",start_flag);
        if(start_flag==1)
	{
			//long_press_reboot_function_setting();
			kernel_restart_ext("hct sd reboot system");
	}
...
static ssize_t kpd_show_kernelpoweroff(struct device_driver *ddri, char *buf)
{
	 sprintf(buf, "%d", poweroff_flag);
	return 1;
}


static ssize_t kpd_store_kernelpoweroff(struct device_driver *ddri, const char *buf, size_t count)
{
	int ret;
	ret = sscanf(buf, "%d", &poweroff_flag);

//	printk("hsl kpd_store_kernelpoweroff %d\n",start_flag);
        if(poweroff_flag==1)
	{
			//long_press_reboot_function_setting();
			kernel_power_off_ext();
	}
	return count;
}

static ssize_t hct_poweronoff_write(struct file *file, const char __user *buf,
						size_t count, loff_t *ppos)
{
        ...
        if(poweroff_flag==1)
	{
        ...
        else if(poweroff_flag==2)
	{
			//long_press_reboot_function_setting();
		kernel_restart_ext("hct sd error reboot system");
	}
	return count;
...
static const struct file_operations fpoweronoff_proc_fops = {
	.write = hct_poweronoff_write,
};
static DRIVER_ATTR(kpd_kernelrestart_interface, S_IWUSR | S_IRUGO, kpd_show_kernelrestart, kpd_store_kernelrestart);
static DRIVER_ATTR(kpd_kernelpoweroff_interface, S_IWUSR | S_IRUGO, kpd_show_kernelpoweroff, kpd_store_kernelpoweroff);
...
#if 0 //defined(__HCT_KERNEL_POWERONOFF_CONTROL_SUPPORT__)
	&driver_attr_kpd_kernelrestart_interface,
	&driver_attr_kpd_kernelpoweroff_interface,
#endif
};
...
#if 0 //defined(__HCT_KERNEL_POWERONOFF_CONTROL_SUPPORT__)
	proc_create("hct_k_poweronoff", 0, NULL, &fpoweronoff_proc_fops);
#endif
	kpd_info("%s Done\n", __func__);
	return 0;
}
...
static int __init kpd_mod_init(void)
{
	...
	if (r) {
		kpd_info("register kpd driver failed (%d)\n", r);
		return r;
	}
#if __HCT_HALL_SUPPORT__
